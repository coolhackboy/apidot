...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { locale } = params;
  const page = await LandingPageService.getLandingPage(locale, "gpt-image-2");
  const path = "/models/gpt-image-2";
  const canonical =
  ...
  const title =
    ...
    `GPT Image 2 API | ${appConfig.appNameInHeader}`;
  const description =
    page.meta?.description?.replace(/PoYo(\.ai)?/g, appConfig.appNameInHeader) ||
    "Generate and edit images with GPT Image 2 on APIDot using prompt, size, and optional image input.";
...
export default async function Page({ params }: PageProps) {
  ...
  const [page, docsPage] = await Promise.all([
    LandingPageService.getLandingPage(locale, "gpt-image-2"),
    LandingPageService.getLandingPage(locale, "docs"),
  ]);
  const path = "/models/gpt-image-2";
  const pageUrl =
    locale === defaultLocale ? `${appConfig.webUrl}${path}` : `${appConfig.webUrl}/${locale}${path}`;
  const modelData = models.find((model) => model.id === "gpt-image-2");
  ...
  const softwareSchema = {
    ...
    description:
      ...
      "Generate and edit images with GPT Image 2 on APIDot.",
    url: pageUrl,
    provider: {
      "@type": "Organization",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: modelData?.name || "GPT Image 2 API", item: pageUrl },
    ],
  };
  ...
  return (
    <>
      ...
      />
      <ClientPage page={page} locale={locale} docsEndpoint={docsPage.docsPage?.endpoints["gpt-image-2"]} />
    </>
  );
}
